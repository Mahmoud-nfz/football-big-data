  ...
  search: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const randomIndex = Math.floor(
        Math.random() * Math.min(imageElements.length, 20),
      );
      const imageLink = $(imageElements[randomIndex]).attr("m"); // Get the 'm' attribute from the randomly selected <a> tag
      let imageUrl: string | null = imageLink
        ? JSON.parse(imageLink).murl
        : null; // Parse the 'm' attribute to extract the 'murl' value
